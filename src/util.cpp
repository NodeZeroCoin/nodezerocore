...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "nodezero" is a composite category enabling all NodeZero-related debug output
            if (ptrCategory->count(std::string("nodezero"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\NodeZero
// Windows >= Vista: C:\Users\Username\AppData\Roaming\NodeZero
// Mac: ~/Library/Application Support/NodeZero
// Unix: ~/.nodezero
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NodeZero";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "nodezero.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty nodezero.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override nodezero.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "nodezerod.pid"));
