...
#if defined(NDEBUG)
#error "NodeZero cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uNZR) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zNZR spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zNZR is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zNZR spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zNZR without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zNZR v1 serial spend not spendable, serial %s, tx %s\n", __func__,
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(v1Serial)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zNZR spend with serial %s from tx %s is not in valid range\n", __func__,
                     spend->getCoinSerialNumber().GetHex(), tx.GetHash().GetHex());
        else
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZNZRModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZNZRModule::validateInput(txin, prevOut, tx, ret)){
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zNZR spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!isPrivZerocoinSpend && !isPublicSpend) {
                    return state.Invalid(error("%s: AcceptToMemoryPool failed for tx %s, every input must be a zcspend or zcpublicspend", __func__,
                                        tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zNZR");
                }
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZNZRModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
            ...
            // Check that zNZR mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
         ...
         nSubsidy = .5 * COIN;
    } else if (nHeight <= 20000 && nHeight > 5000) {
         nSubsidy = 1.2 * COIN;
    } else if (nHeight <= 80000 && nHeight > 20000) {
         nSubsidy = 1.8 * COIN;
    } else if (nHeight <= 140000 && nHeight > 80000) {
         nSubsidy = 2.8 * COIN;
    } else if (nHeight <= 200000 && nHeight > 140000) {
         nSubsidy = 5.2 * COIN;
    } else if (nHeight <= 260000 && nHeight > 200000) {
         nSubsidy = 9.6 * COIN;
    } else if (nHeight <= 300000 && nHeight > 260000) {
         nSubsidy = 10 * COIN;
    } else if (nHeight <= 340000 && nHeight > 300000) {
         nSubsidy = 14 * COIN;
	} else if (nHeight <= 400000 && nHeight > 340000) {
         nSubsidy = 17 * COIN;
	} else if (nHeight <= 435000 && nHeight > 400000) {
         nSubsidy = 24 * COIN;
	} else if (nHeight <= 495000 && nHeight > 435000) {
         nSubsidy = 30 * COIN;
    } else {
         nSubsidy = 40 * COIN;
    }
...
{
    //We aren't using SeeSaw algorithm at this time
    CAmount ret = 0;
    return ret;
}

int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZNZRStake)
{
    int64_t ret = 0;
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZNZRModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from NZR
...
{
    RenameThread("nodezero-scriptch");
...
void RecalculateZNZRMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZNZR..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZNZR..."), percent);
        }
...
void RecalculateZNZRSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZNZR..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZNZR..."), percent);
        }
        ...
        //Remove spends from zNZR supply
...
bool RecalculateNZRSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating NZR supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating NZR supply..."), percent);
        }
...
bool UpdateZNZRSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zNZR supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zNZR mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateNZRSupply(Params().Zerocoin_StartHeight());
    }

    //Track zNZR money supply in the block index
    if (!UpdateZNZRSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zNZR supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zNZRSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    //Check for valid devfee amount and address
    if (!IsDevFeeValid(block, pindex->nHeight))
        return state.DoS(100, error("ConnectBlock() : devfee invalid"), REJECT_INVALID, "bad-devfee-amount");
...
void static UpdateTip(CBlockIndex* pindexNew)
{
     ...
     *
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert NZR to zNZR
...
bool IsDevFeeValid(const CBlock& block, int nBlockHeight)
{
	...
	if (nBlockHeight <= Params().LAST_POW_BLOCK())
        ...
        devfee = blockValue * 0.02; //2%

	bool found = false;

    //if devfee is zero, just pretend we found it
    if (devfee == 0)
        found = true;
    ...
    BOOST_FOREACH (CTxOut out, txNew.vout) {
        if (devRewardscriptPubKey == out.scriptPubKey) {
            ...
            if(out.nValue >= devfee) {
                ...
                LogPrintf("IsDevFeeValid: cannot find the dev fee to dev address in the transaction list.\n");
        }
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zNZR spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zNZR serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> NZRInputs;
        std::vector<CTxIn> zNZRInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zNZRInputs.push_back(stakeIn);
            }else{
                NZRInputs.push_back(stakeIn);
            }
        }
        const bool hasNZRInputs = !NZRInputs.empty();
        const bool hasZNZRInputs = !zNZRInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasNZRInputs)
                    ...
                    for (const CTxIn& NZRIn : NZRInputs){
                        if(NZRIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : NZRInputs) {
            ...
            // Now that this loop if completed. Check if we have zNZR inputs.
            if(hasZNZRInputs){
                for (const CTxIn& zNZRInput : zNZRInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zNZRInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zNZR");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // NodeZero: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zNZR", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zNZR", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
