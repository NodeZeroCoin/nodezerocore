...
public:
    ...
    bool WriteZNZRSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZNZRSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZNZRSeed_deprecated(uint256& seed);
    bool EraseZNZRSeed();
    bool EraseZNZRSeed_deprecated();

    bool WriteZNZRCount(const uint32_t& nCount);
    bool ReadZNZRCount(uint32_t& nCount);
