...
/**
 * NodeZero RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DozNZRSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzNZRseed(const UniValue& params, bool fHelp);
extern UniValue getzNZRseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzNZR(const UniValue& params, bool fHelp);
extern UniValue dzNZRstate(const UniValue& params, bool fHelp);
