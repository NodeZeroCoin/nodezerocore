...
#include "ui_zNZRcontroldialog.h"
...
std::set<std::string> ZNZRControlDialog::setSelectedMints;
std::set<CMintMeta> ZNZRControlDialog::setMints;

bool CzNZRControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZNZRControlDialog::COLUMN_DENOMINATION || column == ZNZRControlDialog::COLUMN_VERSION || column == ZNZRControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZNZRControlDialog::ZNZRControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZNZRControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZNZRControlDialog::~ZNZRControlDialog()
{
    delete ui;
}

void ZNZRControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNZRControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CzNZRControlWidgetItem* itemDenom(new CzNZRControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CzNZRControlWidgetItem *itemMint = new CzNZRControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zNZRTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zNZRTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zNZR.";
            else if (!mint.isSeedCorrect)
                strReason = "The zNZR seed used to mint this zNZR is not the same as currently hold in the wallet";
            else
...
void ZNZRControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNZRControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzNZR_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzNZRControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZNZRControlDialog::GetSelectedMints()
{
...
void ZNZRControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
