...
CzNZRWallet::CzNZRWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zNZR seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNZRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNZRSeed_deprecated()) {
                    LogPrintf("%s: Updated zNZR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zNZR seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zNZR wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNZRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNZRCount(nCountLastUsed);
    else if (!walletdb.ReadZNZRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNZRWallet::Lock()
{
    seedMaster = 0;
}

void CzNZRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNZRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNZR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNZRWallet::LoadMintPoolFromDB()
{
...
void CzNZRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNZRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNZRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zNZRTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNZRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNZR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zNZRTracker which also adds to database
    pwalletMain->zNZRTracker->Add(dMint, true);
...
void CzNZRWallet::SeedToZNZR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNZRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNZRWallet::GenerateDeterministicZNZR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNZRWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNZRWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNZRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
