...
CzNZRTracker::CzNZRTracker(std::string strWalletFile)
{
...
CzNZRTracker::~CzNZRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNZRTracker::Init()
{
...
bool CzNZRTracker::Archive(CMintMeta& meta)
{
...
bool CzNZRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNZRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNZRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNZRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzNZRTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzNZRTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzNZRTracker::GetSerialHashes()
{
...
CAmount CzNZRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNZRTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNZRTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNZRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNZRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNZRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNZRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNZRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNZRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNZRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNZRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNZRWallet* zNZRWallet)
{
    bool iszNZRWalletInitialized = (NULL != zNZRWallet);
    CMintMeta meta;
    ...
    if (! iszNZRWalletInitialized)
        zNZRWallet = new CzNZRWallet(strWalletFile);
    meta.isSeedCorrect = zNZRWallet->CheckSeed(dMint);
    if (! iszNZRWalletInitialized)
        delete zNZRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNZRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNZRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNZRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNZRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNZRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNZRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzNZRWallet* zNZRWallet = new CzNZRWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zNZRWallet);
        }
        delete zNZRWallet;
        LogPrint("zero", "%s: added %d dzNZR from DB\n", __func__, listDeterministicDB.size());
    }
