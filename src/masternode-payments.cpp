...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZNZRStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZNZRStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZNZRStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight+1, blockValue, 0, fZNZRStake);
    ...
    if (hasPayment) { //masternode to pay found
        if (fProofOfStake) {
            ...
            //subtract mn payment and devfee from the stake reward
            if (!txNew.vout[1].IsZerocoinMint())
                if (i == 2) {
                    // Majority of cases; do it quick and move on
                    txNew.vout[i - 1].nValue -= (masternodePayment + devfee);
                } else if (i > 2) {
                    // special case, stake is split between (i-1) outputs
                    unsigned int outputs = i-1;
                    CAmount mnPaymentSplit = (masternodePayment + devfee) / outputs;
                    CAmount mnPaymentRemainder = (masternodePayment + devfee) - (mnPaymentSplit * outputs);
    ...
    } else { //No masternode to pay found
        if (!fProofOfStake)
                txNew.vout[0].nValue = blockValue - masternodePayment - devfee; //subtract masternodepayment just to keep code happy during POW
        else {//POS with no MN
            unsigned int k = txNew.vout.size();
			txNew.vout[k-1].nValue -= devfee;
        }
    }
